...
<%
 //사용자 요청값(String[])을 contains메소드 사용을 위해 list로 변환 
 	...
 	List<String> deptCodeList = deptCodeArr != null ? Arrays.asList(deptCodeArr) : null;
...
<div id="emp-container">
			...
			<table id="tbl-search">
				...
				<tr>
					...
					<td>
						...
						<input type="checkbox" name="jobCode" id="${map.jobCode}" value="${map.jobCode}" 
				...
				<!-- 실습문제 : 부서 조회 (직급조회와 모두 일치 하는 사원 ) 
					input:cheakbox + label 는 3개마다 개행 할 것
					인턴 사원(D0)도 조회될 수 있도록 할 것  ->  값은 이미 deptCode Array에 받아서 mapper에서 처리
				
				foreach문 안에 var는 다른 것과 같아도 상관 엊ㅅ다
				-->
				<tr>
					<th>부서</th>
					<td>
					<c:forEach items="${deptList}" var="deptmap" varStatus="vs">						
						<input type="checkbox" name="deptCode" id="${deptmap.deptCode}" value="${deptmap.deptCode}"
						${deptCodeList.contains(deptmap.deptCode)? 'checked' : '' }/>
						<label for="${deptmap.deptCode}">${deptmap.deptTitle}</label>		
	...
	<table class="tbl-emp">
		...
		<tr>
			<td>${vs.count}</td>
			<td>
				<a href="${pageContext.request.contextPath}/emp/updateEmp.do?empId=${emp['EMP_ID']}">
				${emp['EMP_ID']}
				</a>
			</td>
			<td>${emp['EMP_NAME']}</td>
